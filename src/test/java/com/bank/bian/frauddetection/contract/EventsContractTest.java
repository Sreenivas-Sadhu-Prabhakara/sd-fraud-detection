package com.bank.bian.frauddetection.contract;
...
class EventsContractTest {
    ...
    void codeTopicsAreDeclaredInTheEventContract() throws Exception {
        ...
        assertThat(events.get("service")).isEqualTo("sd-fraud-detection");
        Set<String> topics = ((List<Map<String, Object>>) events.get("publishes")).stream()
                .map(t -> String.valueOf(t.get("topic")))
                .collect(Collectors.toSet());
        assertThat(topics).as("FraudDetectionService.TOPIC_ALERTS must be a declared publish topic").contains(FraudDetectionService.TOPIC_ALERTS);
